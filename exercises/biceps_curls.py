    ...
    while cap.isOpened():
        ...
        try:
            if results.pose_landmarks:
                ...
                # Apply smoothing to the wrist position
                if previous_wrist_y is not None:
                    smoothed_wrist_y = (smoothing_factor * left_wrist.y) + ((1 - smoothing_factor) * previous_wrist_y)
                else:
                    smoothed_wrist_y = left_wrist.y

                previous_wrist_y = smoothed_wrist_y

                # Logic for detecting bicep curls using smoothed wrist data
                if smoothed_wrist_y > left_elbow.y:  # Arm is down
                    stage = "down"
                if smoothed_wrist_y < left_elbow.y and stage == 'down':  # Arm is up
                    stage = "up"
